...
import { deletePlaybook, getPlaybookList } from '@/api/modules/playbook'
...
const tableOptions = ref([
  ...
  { label: '内含指令数', prop: 'commandCount', width: '150' },
  { label: '启用状态', prop: 'isEnabled', width: '200' },
  { label: 'Cron表达式', prop: 'cronExpression', width: '250' },
...
async function loadData() {
  ...
  try {
    const params = {
      playbookName: searchForm.value.playbookName,
      page: page.value,
        ...
        <div class="table-card">
          ...
          <CommonTable v-loading="dataLoading" :list="dataList" :options="tableOptions">
            ...
            <template #cronExpression="{ row }">
              <span class="text-sm text-gray-600">{{ row.cronExpression || '-' }}</span>
            </template>
            <template #action="{ row }">
