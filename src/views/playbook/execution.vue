      ...
      <div v-else class="page-main">
        <div class="overview-card">
          ...
          <el-descriptions :column="3" border>
            <el-descriptions-item label="执行ID">{{ executionData.executionId }}</el-descriptions-item>
            <el-descriptions-item label="状态">
              <el-tag :type="statusTagType(executionData.status)">{{ executionData.status }}</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="耗时">{{ executionData.elapsedTime }}</el-descriptions-item>
            <el-descriptions-item label="目标主机">
              <el-tag v-for="host in executionData.targetHosts" :key="host" class="mr-1">{{ host }}</el-tag>
            </el-descriptions-item>
        ...
        <div class="results-card">
          <h3 class="text-lg font-semibold mb-4">执行详情</h3>
          <div v-for="(hostResult, index) in executionData.results" :key="index" class="host-result-group">
            <h4 class="text-md font-bold p-2 bg-gray-100 dark:bg-gray-800 rounded">主机: {{ hostResult.host }}</h4>
            <div v-for="(step, stepIndex) in hostResult.steps" :key="stepIndex" class="step-item">
              <div class="step-header">
                <strong>{{ step.command }}</strong>
                <el-tag :type="statusTagType(step.status)" size="small">{{ step.status }}</el-tag>
              </div>
...
import { getPlaybookExecution, getPlaybookDetail } from '@/api/modules/playbook'
